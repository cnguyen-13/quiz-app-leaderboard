...
async function getQuestions(difficulty: DifficultyType, category: string, callback: React.Dispatch<React.SetStateAction<QuestionType[]>>) {
    //Fetch Data
    const data = await (await fetch(`https://opentdb.com/api.php?amount=${TOTAL_QUESTIONS}&difficulty=${difficulty}${category}`)).json();
    ...
    const questions: QuestionType[] = questionsArray.map(question => {
        question['answer_choices'] = shuffleArray([...question['incorrect_answers'], question['correct_answer']]);
        return question;
    })
    callback(questions);
}
